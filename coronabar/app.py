...
class CoronaBar(object):
    ...
    default_country = location.get_loc()
    update_interval = 60  # seconds
    about_url = "https://coronavirus.jhu.edu/map.html"

    # APP
    def __init__(self):
        self.app = rumps.App("Corona Bar", "🦠")
        self.countries = rumps.MenuItem(title="Select Country")
        self.map = rumps.MenuItem(title="JHU COVID-19 Dashboard", callback=self.open_page)

        self.country = self.default_country

        self.app.menu = [self.countries, self.map]
    ...
    def update_country_listing(self, country):
        ...
        for k, v in self.app.menu.items():
            if k not in ["Select Country", "Quit", "JHU COVID-19 Dashboard"]:
                del self.app.menu[k]
